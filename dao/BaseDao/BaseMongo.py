...
class BaseMongo(Base):
    ...
    def find_one(self, *args, **kwargs):
        ...
        with self.conn.getMongoConn(city=city, db_name=db_name) as client:
            table = client.get_collection(self.table)
    ...
    def find_page(self, *args, **kwargs):
        ...
        with self.conn.getMongoConn(city=city, db_name=db_name) as client:
            ...
            if field:
                data = table.find(filter,field)\
                    .skip((page.get("index") -1) * page.get("size"))\
                    .sort(sort)\
                    .limit(page.get("size"))
            else:
    ...
    def getMongoNextId(self, key):
        ...
        with self.conn.getMongoConn(city='') as client:
            ...
            if count == 0:
                ...
                mongo_id = table.find({"%s_id" % key: {"$ne": 9999999}}).limit(1).sort('%s_id' % key, -1)[0][
                               '%s_id' % key] + 1
            return mongo_id
